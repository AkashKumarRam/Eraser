...
function SideNavTopSection({ user,setActiveTeamInfo }: any,) {
    ...
    return (
        <div>
                ...
                <PopoverContent className='ml-7 p-4'>
                    <div>
                        {teamList?.map((team, index) => (
                            <h2 className={`p-2 hover:bg-blue-500 hover:text-white rounded-lg mb-2 cursor-pointer ${activeTeam?._id === team._id && "bg-blue-500 text-white"}`} key={index} onClick={() => setActiveTeam(team)}>{team.teamName}</h2>
                        ))}
                    </div>
                    ...
                    {user && <div className='mt-2 flex gap-2 items-center'>
                        <Image src={user?.picture} alt='user-profile-logo' width={30} height={30} className='rounded-full' />
            ...
            <Button variant="outline" className='w-full justify-start gap-2 font-bold mt-8 bg-gray-100'><LayoutGrid className='h-5 w-5'/> All Files</Button>
        </div>
